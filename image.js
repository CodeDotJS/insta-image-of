...
const argv = require('yargs')
	...
	.usage(colors.cyan.bold('\nUsage : $0 -l [full-link] -n [image-name]'))

	.demand(['l', 'n'])

	.describe('l', colors.cyan.bold('❱ ') + ' full link to download image')
...
function parseArgument(mainLink) {

	const res = /instagram.com/gi;

	const newLink = mainLink.replace(res, '').replace('www.', '').replace('https://', '').replace('/p/', '').replace('/', '');
	...
	res.on('end', () => {
		...
		if (regMatches && regMatches[0]) {
			...
			https.get(remChars, res => {
				...
				console.log(colors.cyan.bold('\n ❱ Image Saved In        : '), ' ', colors.green.bold(savedIn), colors.cyan.bold('❱'), colors.green.bold(argv.n + '.jpg\n'));
			}).on('error', err => {
				console.log(err);
